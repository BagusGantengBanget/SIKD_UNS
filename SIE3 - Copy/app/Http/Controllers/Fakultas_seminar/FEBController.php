...
class FEBController extends Controller
{
    ...
    public function index(Request $request)
    {
       ...
       ->get()->count();
        ...
        return view('fakultas_seminar_FEB.index'  ,compact( ['data'], 'keyword','categoriesval', 'dataval', 'categoriescat', 'datacat', 'tahunpen',
...
/* $query = DB::select('SELECT `dosen`.`id_dosen` AS `ID_DOSEN`, 
                            ...
                            `kinerja_seminar`.`judul_seminar` AS `JUDUL`, 
                            `kinerja_seminar`.`penerbit` AS `PUBLISER`, 
                            `kinerja_seminar`.`tahun` AS `TAHUN`, 
                            `kinerja_seminar`.`isbn` AS `ISBN_ISSN`, 
                            `bidang_kajian`.`nama_kajian` AS `BIDANG_KAJIAN`, 
                            `bidang_ilmu`.`nama_bidangilmu` AS `BIDANG_ILMU`,  
                            `kinerja_seminar`.`status_seminar` AS `STATUS`, 
                            `REF_UNIT`.`ID` AS `ID_UNIT`, 
                            `REF_UNIT`.`NAMA` AS `JURUSAN`,
                            `REF_UNIT_FAKULTAS`.`FAKULTAS` AS `FAKULTAS` 
                            FROM
                            (((((((( `kinerja_seminar_author` JOIN `kinerja_seminar` ON (( `kinerja_seminar`.`id_seminar` = `kinerja_seminar_author`.`id`))) 
                            JOIN `dosen` `dosen` ON ( ( `kinerja_seminar_author`.`nidn` = `dosen`.`nidn` )))
                            ...
                            LEFT JOIN `bidang_kajian` ON ( ( `kinerja_seminar`.`id_kajian` = `bidang_kajian`.`id_kajian` ))) 
                            LEFT JOIN `bidang_ilmu` ON ( ( `kinerja_seminar`.`id_bidangilmu` = `bidang_ilmu`.`id_bidangilmu` ))) ');
