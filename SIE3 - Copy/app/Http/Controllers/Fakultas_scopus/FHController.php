...
class FHController extends Controller
{
    ...
    public function index(Request $request)
    {
       ...
       ->get()->count();
        ...
        return view('fakultas_scopus_FH.index'  ,compact( ['data'], 'keyword','categoriesval', 'dataval', 'categoriescat', 'datacat', 'tahunpen',
...
/* $query = DB::select('SELECT `dosen`.`id_dosen` AS `ID_DOSEN`, 
                            ...
                            `kinerja_scopus`.`judul_scopus` AS `JUDUL`, 
                            `kinerja_scopus`.`penerbit` AS `PUBLISER`, 
                            `kinerja_scopus`.`tahun` AS `TAHUN`, 
                            `kinerja_scopus`.`isbn` AS `ISBN_ISSN`, 
                            `bidang_kajian`.`nama_kajian` AS `BIDANG_KAJIAN`, 
                            `bidang_ilmu`.`nama_bidangilmu` AS `BIDANG_ILMU`,  
                            `kinerja_scopus`.`status_scopus` AS `STATUS`, 
                            `REF_UNIT`.`ID` AS `ID_UNIT`, 
                            `REF_UNIT`.`NAMA` AS `JURUSAN`,
                            `REF_UNIT_FAKULTAS`.`FAKULTAS` AS `FAKULTAS` 
                            FROM
                            (((((((( `kinerja_scopus_author` JOIN `kinerja_scopus` ON (( `kinerja_scopus`.`id_scopus` = `kinerja_scopus_author`.`id`))) 
                            JOIN `dosen` `dosen` ON ( ( `kinerja_scopus_author`.`nidn` = `dosen`.`nidn` )))
                            ...
                            LEFT JOIN `bidang_kajian` ON ( ( `kinerja_scopus`.`id_kajian` = `bidang_kajian`.`id_kajian` ))) 
                            LEFT JOIN `bidang_ilmu` ON ( ( `kinerja_scopus`.`id_bidangilmu` = `bidang_ilmu`.`id_bidangilmu` ))) ');
