...
class SVController extends Controller
{
    ...
    public function index(Request $request)
    {
       ...
       ->get()->count();
        ...
        return view('fakultas_jurnal_SV.index'  ,compact( ['data'], 'keyword','categoriesval', 'dataval', 'categoriescat', 'datacat', 'tahunpen',
...
/* $query = DB::select('SELECT `dosen`.`id_dosen` AS `ID_DOSEN`, 
                            ...
                            LESV JOIN `bidang_kajian` ON ( ( `kinerja_jurnal`.`id_kajian` = `bidang_kajian`.`id_kajian` ))) 
                            LESV JOIN `bidang_ilmu` ON ( ( `kinerja_jurnal`.`id_bidangilmu` = `bidang_ilmu`.`id_bidangilmu` ))) ');
