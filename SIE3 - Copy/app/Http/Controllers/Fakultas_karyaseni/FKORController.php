...
class FKORController extends Controller
{
    ...
    public function index(Request $request)
    {
       ...
       ->get()->count();
        ...
        return view('fakultas_karyaseni_FKOR.index'  ,compact( ['data'], 'keyword','categoriesval','dataval', 'categoriescat', 'datacat','tahunpen', 
...
/* $query = DB::select('SELECT `dosen`.`id_dosen` AS `ID_DOSEN`, 
                            ...
                            LEFT JOIN `bidang_kajian` ON ( ( `kinerja_karyaseni`.`id_kajian` = `bidang_kajian`.`id_kajian` ))) 
                            LEFT JOIN `bidang_ilmu` ON ( ( `kinerja_karyaseni`.`id_bidangilmu` = `bidang_ilmu`.`id_bidangilmu` ))) ');
