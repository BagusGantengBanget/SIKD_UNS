...
//===================================
       ...
       ->get()->count();
        ...
        return view('fakultas_pembicara_FMIPA.index'  ,compact( ['data'], 'keyword','categoriesval', 'dataval', 'categoriescat', 'datacat','tahunpen', 
...
/* $query = DB::select('SELECT `dosen`.`id_dosen` AS `ID_DOSEN`, 
                            ...
                            LEFMIPA JOIN `REF_GOLONGAN_RUANG` `g` 
                            ...
                            LEFMIPA JOIN `bidang_kajian` ON ( ( `kinerja_pembicara`.`id_kajian` = `bidang_kajian`.`id_kajian` ))) 
                            LEFMIPA JOIN `bidang_ilmu` ON ( ( `kinerja_pembicara`.`id_bidangilmu` = `bidang_ilmu`.`id_bidangilmu` ))) ');
