...
class FIBController extends Controller
{
    ...
    public function index(Request $request)
    {
       ...
       ->get()->count();
        ...
        return view('fakultas_program_FIB.index'  ,compact( ['data'], 'keyword','categoriesval','dataval', 'categoriescat', 'datacat', 'tahunpen',
...
/* $query = DB::select('SELECT `dosen`.`id_dosen` AS `ID_DOSEN`, 
                            ...
                            `kinerja_program`.`judul_program` AS `JUDUL`, 
                            `kinerja_program`.`penerbit` AS `PUBLISER`, 
                            `kinerja_program`.`tahun` AS `TAHUN`, 
                            `kinerja_program`.`isbn` AS `ISBN_ISSN`, 
                            `bidang_kajian`.`nama_kajian` AS `BIDANG_KAJIAN`, 
                            `bidang_ilmu`.`nama_bidangilmu` AS `BIDANG_ILMU`,  
                            `kinerja_program`.`status_program` AS `STATUS`, 
                            `REF_UNIT`.`ID` AS `ID_UNIT`, 
                            `REF_UNIT`.`NAMA` AS `JURUSAN`,
                            `REF_UNIT_FAKULTAS`.`FAKULTAS` AS `FAKULTAS` 
                            FROM
                            (((((((( `kinerja_program_author` JOIN `kinerja_program` ON (( `kinerja_program`.`id_program` = `kinerja_program_author`.`id`))) 
                            JOIN `dosen` `dosen` ON ( ( `kinerja_program_author`.`nidn` = `dosen`.`nidn` )))
                            ...
                            LEFT JOIN `bidang_kajian` ON ( ( `kinerja_program`.`id_kajian` = `bidang_kajian`.`id_kajian` ))) 
                            LEFT JOIN `bidang_ilmu` ON ( ( `kinerja_program`.`id_bidangilmu` = `bidang_ilmu`.`id_bidangilmu` ))) ');
